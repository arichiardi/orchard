(ns orchard.meta
  ...
  (:require
   ...
   #?(:cljs [orchard.cljs.meta :as cljs-meta]))
  #?(:clj
     (:import
...
(defn- maybe-add-spec
  ...
  [v meta-map]
  (if-let [spec (when v #?(:clj (spec/spec-form (var-name v))))]
...
(defn special-sym-meta
  ...
  (let [orig-sym sym
    ...
    (when-let [m (and compiler-special? #?(:clj (#'repl/special-doc sym)
                                           ;; the following is for bootstrap cljs - jvm clj is above
                                           :cljs (cljs-meta/special-doc sym)))]
      (-> m
          (assoc :name orig-sym)
          maybe-add-url))))
...
(def special-forms
  ...
  (concat (keys #?(:clj (var-get #'repl/special-doc-map)
                   ;; the following is for bootstrap cljs - jvm clj is above
                   :cljs cljs-meta/special-doc-map))
          '[& catch finally]))
